 ...
 */
export function normalizeCsvColumns(columns: string[]): Record<string, string> {
  ...
  columns.forEach((col) => {
    ...
    normalized[col] = columnMap[key] || col;
  });
 ...
 */
export function validateCsvStructure(columns: string[]): {
  valid: boolean;
  missingColumns: string[];
} {
  const requiredColumns = ['NOMBRE', 'USUARIO', 'HORARIOS', 'CONTRATO'];
  const upperColumns = columns.map((c) => c.toUpperCase().trim());

  const missing = requiredColumns.filter(
    (col) => !upperColumns.includes(col)
  );
